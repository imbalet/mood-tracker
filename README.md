...
В `.env` обязательно замените `BOT_TOKEN` и `DB_PASS`.
...
удаляет контейнер и test volume после завершения. Миграции получают явный
тестовый DSN и перед выполнением проверяют имя БД, поэтому не используют
локальные `DB_*` из `.env`. Mypy проверяет только `src/`; unit и integration
